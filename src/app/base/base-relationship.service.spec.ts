...
describe('BaseRelationshipService', () => {
  ...
  it('index should return a list of relationships', (done: DoneFn) => {
    const relationship2: BaseRelationship = {
      model1Id: '2',
      model1Name: 'Model 1',
      model2Id: '2',
      model2Name: 'Model 2',
    ...
    const relationships = new Array<BaseRelationship>(relationship1, relationship2);
    const relationshipsGridDataResult: GridDataResult = {
      data: relationships,
    ...
    relationshipService
      ...
      .subscribe((result: GridDataResult) => {
        expect(result).toEqual(relationshipsGridDataResult, 'expected relationships');
        expect(httpBaseRelationshipSpy.get.calls.count()).toBe(1, 'one call');
        done();
      });
  });

  it('details should return a relationship', (done: DoneFn) => {
    ...
    relationshipService
      ...
      .subscribe((result: BaseRelationship) => {
        expect(result).toEqual(relationship1, 'expected relationship1');
        expect(httpBaseRelationshipSpy.get.calls.count()).toBe(1, 'one call');
        done();
      });
  });

  it('create should return a relationship', (done: DoneFn) => {
  ...
  afterEach(() => {
    httpBaseRelationshipSpy = undefined;
