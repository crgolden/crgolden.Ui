...
export abstract class BaseModelService<T extends BaseModel> {
  ...
  createRange$(models: Array<T>): Observable<Array<T>> {
    return this.http
      .post<Array<T>>(`${environment.apiUrl}/${this.controllerName}/create-range`, JSON.stringify(models), {
        headers: this.headers
      });
  }
  ...
  editRange$(models: Array<T>): Observable<Object> {
    return this.http
      .put(`${environment.apiUrl}/${this.controllerName}/edit-range`, JSON.stringify(models), {
        headers: this.headers
      });
  }
